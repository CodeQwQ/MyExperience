...
*thisModule*.resolveTemp(var, target_pattern_name)可以获得转换后的元素，var是源模型变量， target_pattern_name是目标模型变量名字（在相对应的rule里面声明的），用字符串''形式给出
...
association底下生成property失败原因：对memberEnd进行赋值，而memberEnd是只读的，真实的属性在ownedEnd,**以后对属性赋值一定要参考文档**
