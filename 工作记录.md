...
1. ```sql
   SELECT COUNT(*) FROM (SELECT DISTINCT e.equipment_code, e.equipment_name FROM sys_equipment e WHERE NOT EXISTS (SELECT 1 FROM sys_equipment_stock s WHERE s.equipment_code = e.equipment_code) AND NOT EXISTS (SELECT 1 FROM (SELECT s.*, t.location FROM sys_equipment_stock s LEFT JOIN sys_task t ON s.task_code = t.task_code WHERE s.equipment_code = e.equipment_code ORDER BY s.equipment_date DESC, CASE WHEN s.type = '出库' THEN 1 ELSE 0 END DESC LIMIT 1) AS last_record WHERE last_record.type = '出库')) TOTAL
   ```
