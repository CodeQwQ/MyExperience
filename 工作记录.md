...
2. 阅读压实度鉴定表生成逻辑：（3）
   ...
   3. 接着改分离上下面层的，这部分比较麻烦，很多部分使用的处理函数不一样**4.8**
...
1. JTL环境会自动序列化元模型成ASP，JTL转换脚本也会有ATl语言转换成ASP（ASP负责描述问题）；随后使用DLV去进行问题求解从而转换出模型
2. 用户只需要写JTL转换脚本，其他晦涩的ASP交给内核
3. 可以在Eclipse找到开发环境
4. GPT分析先导入好元模型再设计好用例图到类图和活动图的关系映射就可以用（本来也是关系式的语言）

放弃阅读案例2，因为它是元模型变更后如何适应新模型，但是我们目前还不需要更新元模型迭代，但是这个方法以后可以加上
...
1. 回顾第一篇论文：
   ...
   1. 有规则，对用例描述进行分析，什么变成类什么变成活动（动词名词）
      问题是：我们的用例描述是DSL，如果像是之前展示的类似结构体或者XML，感觉应该是难度降低了，因为没有自然语言的二义性

2. 现在方法其实很确定了（不知道有没有因为看的少限制），寻找开源工具就好了：

   1. github无
   2. 大模型搜索（deepseek、gpt）无，但是建议可以ATL编写转换规则

3. 研究转换规则，如何对应：

   1. 安装环境

      1. Papyrus最后使用RCP包进行开发

   2. 按照GPT步骤进行例子跑通：

      1. 能创建工程，但是看不到创建的UML文件，而且没办法复制到ATL工程的input文件夹

   3. 5.21（6） 找了个example walkthough进行学习，大致流程如下：

      1. 创建ATL工程
      2. 创建元模型（源和目标）
      3. 创建ATL转换代码
      4. 创建实例（基于元模型），就可以运行ATL文件进行转换了

   4. 创建XMI文件（元模型）失败：文件提前结束

      1. 使用text editor进行打开就能看到，不然只能看到报错
      2. XMI文件报错解决

   5. 创建ATL文件

      1. 报错解决
         1. 一些莫名其妙的中括号和分号，尝试跳过，因为报错的点实在太奇怪，在一个字母中间或者空白，有可能是IDE没做好。最后解决不了，更换大模型成gemini
         2. 这个语法报错解决不了就没办法生成.asm文件

   6. 以代码例子学习ATL语法，达到能看懂的程度

      ```
      -- rule像是主逻辑，helper像是函数
      -- 执行顺序：
      启动转换
      ├── 初始化所有唯一规则(unique rule)
      ├── 匹配主规则(matched rule)的源元素
      │    ├── 执行规则内helper
      │    └── 生成目标元素
      └── 惰性规则(lazy rule)按需执行
      -- ATL模块头定义
      module UseCase2ClassDiagram;
      ...
      -- 规则2：UseCase转Class（带操作方法）
      rule UseCaseToServiceClass {
          from
              uc : UML!UseCase
          to
              c : UML!Class (
                  name <- uc.name + 'Service',
                  ownedOperation <- Sequence{
                      thisModule.createOperation('execute' + uc.name)
                  }
              )
      }
      
      -- 规则3：Association转ClassRelationship（双向关联）
      rule AssociationToClassAssociation {
          from
              assoc : UML!Association
          to
              rel : UML!Association (
              -- memberEnd是Association的属性
              -- collect 则把他们组合起来
              -- resolveTemp是调用转换后的模型
                  memberEnd <- assoc.memberEnd->collect(e | 
                      thisModule.resolveTemp(e.type, 'c', 'c')
                  )
              )
      }
      
      -- 辅助规则：创建操作方法
      -- context UML!UseCase表示仅适用于UseCase实例
      helper context UML!UseCase def: createOperation(opName : String) : UML!Operation {
          return new UML!Operation(
              name <- opName,
              visibility <- #public,
              ownedParameter <- Sequence{new UML!Parameter(direction <- #in, type := thisModule.StringType)}
          )
      }
      
      -- 类型映射辅助定义
      helper def: StringType : UML!Type {
          return UML!Class.allInstancesFrom('OUT')->any(name = 'String')
      }
      ```
   ...
   1. 能看懂代码，但是解决不了错误
      1. helper放到不同的位置有不同的错误
      2. 把helper全删了并把有调用helper的rule删了，没报错，但是运行的时候报错有一个函数调用的参数个数错误
      3. 把参数错误的rule也删了就跑通了
...
<img src="${imagines}/image-20250526153032815.png" alt="image-20250526153032815" style="zoom:80%;" />





*thisModule*.resolveTemp(var, target_pattern_name)可以获得转换后的元素，var是源模型变量， target_pattern_name是目标模型变量名字（在相对应的rule里面声明的），用字符串''形式给出。该函数意义：对于源模型的联系两端，则需要转换后的对应两端

```
rule ActorToClass {
  … to
    actorClass : UML!Class { … }
}

rule UseCaseToServiceClass {
  … to
    serviceClass : UML!Class { … }
}

rule AssociationToClassAssociation {
  … memberEnd <- assoc.memberEnd->collect(e |
        thisModule.resolveTemp(e.type, 'actorClass')
      )
      ->union(
        assoc.memberEnd->collect(e |
          thisModule.resolveTemp(e.type, 'serviceClass')
        )
      )
}
...
在上述示例中，`self.name` 等价于 `uc.name`，用于获取当前 `UseCase` 元素的名称。

```
helper context UML!Class def: isService() : Boolean =
  self.name.endsWith('Service');
```

此处，`self` 表示调用 `isService()` 函数的 `UML!Class` 实例。



有各种数据类型的的operation直接查找该文档https://wiki.eclipse.org/ATL/User_Guide_-_The_ATL_Language#OclType_operations

这里说一个比较特别的操作：source->operation_name(iterators | body)：
operation_name就是对source进行一些操作，例如collecting the names of all MOF classes: 
...
helper里面不支持对象模板构造，只允许OCL表达式





最后下载了个UML profile转UMLClass的项目参考，把rule里面的代码分开写，避免了对象模板，还是有问题

5.27（6）

把#in改成了#in_dir报错解决，所以还是得去看源ecore定义，但是我看的明明就是#in,后面需要确定下版本，现在先解决没有asm文件的问题

查找UML 的ecore版本的时候发现，是因为eclipe编辑器里面默认不显示继承的属性，所以有些属性没能看到

解决association的转换问题，resolveTemp似乎陷入循环.最终使用了一个调用rule，但是在association的转换当中依然报错找不到方法
...
问题：平台问题，Android、鸿蒙5.0、ios？

1. 聊天框及其功能（语音通话、视频通话、视频传输）：有许多开源SDK
   1. flutter集成语音通话的SDK（Agora SDK、Twilio Programmable Voice SDK、CometChat Calls SDK）
   2. 按照SDK文档进行搭载
   3. 与后端进行连接，后端不做语音处理，仅仅做一个状态更新、通知、记录作用
   4. 即可发起通话
2. 地图大屏：需要知道大屏系统的后端地图使用地图API
3. 开发框架： （谷歌）flutter前端（可以直接部署在Android和ios），后端任意，spring boot  ，flask， Django都可以。serverpod专门为flutter服务作后端（Dart语言）

### association转化解决（4）

改为lazy rule代码可以正确运行（但是依然没有生成在association下面），所以called rule 和lazy rule的区别在哪？

1. 读完文档也不知道区别在哪，用法却是一样的，用法除了lazy有源模型匹配而called没有源模型匹配外并无其他区别了
2. 以后就使用lazy rule当作调用

组会（1）

5.29（

association底下生成property失败原因：对memberEnd进行赋值，而memberEnd是只读的，真实的属性在ownedEnd,**以后对属性赋值一定要参考文档**（6）
...
rule UseCaseToServiceClass {
    ...
    to
        serviceClass : UML!Class (
            name <- uc.name + 'Service',
            ownedOperation <- operationc
        ),
		operationc : UML!Operation(
					name           <- uc.name + 'Operation',
    				visibility     <- #public
		)
    				
}

-- 为了保证memberend的类型依然是property的不得已规则
lazy rule Property2Property {
  ...
  to
    prop : UML!Property (
      name <- p.name,
      type <-
	  	if p.type.oclIsTypeOf(UML!Actor)
		then
	  		thisModule.resolveTemp(p.type, 'actorClass')
	  	else thisModule.resolveTemp(p.type, 'serviceClass')
		endif
...
rule AssociationToClassAssociation {
    ...
    to
        rel : UML!Association (
			ownedEnd  <- assoc.memberEnd->collect(p |
        thisModule.Property2Property(p))
		)
}
```
...
1.  chat sdk语音视频也要自己集成
2. V_CHAT_SDK只能聊天，语音通话自行集成WebRTC
3. openIM 可以聊天可以视频
4. Jitsi Meet是个面向会议的
5. Rocket.Chat即时消息、语音和视频通话
6. Matrix + Element，Matrix 开放的通信协议，Element是其官方客户端
...
| 组件                  | 配置示例                                        | 预估月费用（人民币） |
| --------------------- | ----------------------------------------------- | -------------------- |
| Rocket.Chat + MongoDB | 4 核 vCPU + 8 GB RAM + 200 GB SSD + 5 Mbps 带宽 | ¥400 ~ ¥600 / 月     |

假设条件： 500 个注册用户、50 人同时在线（含语音/视频通话）并且需要存储文字和视频消息

**步骤**“：docker部署（很便捷）

#### 开发环境

后端：如果需要定制服务端（比如转报告、报送）vs code 克隆服务端仓库，部署就要手动部署了

前端：Android Studio克隆官方 React Native 仓库，对应的页面进行修改即可

#### 社区版和商业版对比

| 功能模块               | 社区版（Community）         | 商业版（Enterprise）                                         |
| ---------------------- | --------------------------- | ------------------------------------------------------------ |
| **用户认证**           | 支持 LDAP/SAML/标准 REST    | 支持更多 SSO（CAS、OpenID Connect、Kerberos、ADFS 等），自动同步 |
| **单点登录 (SSO)**     | 基础 SAML/LDAP              | 多种企业级 SSO + 可定制 SSO 登录界面                         |
| **权限管理**           | 基本角色(Role) 与频道权限   | 更细粒度角色体系、按部门/组织继承、临时权限、自动化策略      |
| **审计与合规**         | 无审计日志                  | 完整审计日志、合规导出、DLP/敏感词屏蔽                       |
| **高可用 (HA) / 集群** | 单机版，无自动集群方案      | 官方集群部署支持（Neo4J／Cassandra／MongoDB 副本集方案）、负载均衡 |
| **运维支持**           | 社区论坛、自助文档          | 7×24 官方技术支持、升级脚本、运维最佳实践、紧急补丁          |
| **White-Label 品牌化** | 部分更改 logo（需改源码）   | 管理后台一键设置 Logo、主题色、登录页面品牌、邮件模板        |
| **视频会议**           | Jitsi 集成（自建/公有均可） | 高可用视频集群、多平台会议（Zoom/Webex 自定义集成）、屏幕共享录制 |
| **Livechat 客服功能**  | 基础访客邀请、转接          | 多队列管理、SLA 规则、机器人自动回复、访客评分、质检报表     |
| **移动端定制**         | 可在 React Native 源码修改  | 提供深度品牌化包、App Store/Play Store 管理支持              |
| **数据加密**           | 传输层加密 (TLS)            | 支持端到端加密 (E2EE)、数据库加密、密钥管理                  |
| **企业集成**           | 基础 Webhook / REST API     | 预置多个企业级插件（Salesforce、Workday、ServiceNow 等）     |
| **报表与监控**         | 基础用户在线数、消息量统计  | 专业报表引擎（活跃度分析、渠道使用率、业务 KPI）、Prometheus/Grafana 可视化 |
| **培训与咨询**         | 无                          | 官方培训、架构评审、性能咨询、安全审核服务                   |

### 

### papyrus工具测试（1.5）

1. bi文件就是图形界面，uml就是结构保存
2. 可以在一个papyrus项目内创建多个papyrus模型，一个的papyrus模型就是一个uml
3. 虽然直接复制papyrus的uml可能会展示下面的底层元模型信息，但是转换的时候不会连带着转换，不知道为什么

6.4

1. 用例转类图：
   1. 用户直接转类，只有名字
   2. 用例转类，带操作方法，方法名字就是用例名+operation
   3. 连接双方转成目标模型
2. 寻找用例描述到活动图的方法：（1）

### 论文 **An Automated Approach to Transform Use Cases into Activity Diagrams**  

会议：Model Driven Engineering Languages and Systems 2010

#### 总体流程：

RUCM（Restrict User Case Modeling) --> aToucan工具（内部集成NLP[Stanford Parser]进行辅助）：通过词法 和 语法分析分类句子、对类内句子进行UCMeta构建（2）

#### RUCM和自然语言区别

RUCM是一种对**用例文本语言进行限制和规范的建模方法**，在之前的论文中已经阅读过类似方法，就是约束了自然语言使其结构化。

**例子**：

| 项目         | 自然语言用例                                                 | RUCM 版本                                                    |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 用户插入卡   | “When the customer approaches, he puts the card in the slot.” | “The ATM customer INSERTS the card.”                         |
| 系统验证 PIN | “The ATM checks whether the PIN is correct.”                 | “The system VALIDATES THAT the PIN is correct.”              |
| 中止流程     | “In case of wrong PIN three times, the process stops.”       | “ABORT.”                                                     |
| 包含子用例   | “At this point, Validate PIN use case is called.”            | “INCLUDE USE CASE Validate PIN.”                             |
| 条件流程     | “If the funds are insufficient, print message.”              | “IF funds are insufficient THEN print insufficient funds message.” |

本文一开始默认RUCM是已经构建好的。2010年没有什么很好的办法去处理，只能手动处理，但是现在大模型能够很好解决。（3）
